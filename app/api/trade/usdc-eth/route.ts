...
export async function POST(request: Request) {
  ...
  try {
    ...
    let trade = await userWallet.createTrade({
        amount: 0.00001,
    ...
    if (trade.getStatus() === 'complete') {
        ...
        return (Response as any).json(trade.toString())
      } else {
        console.log(`Trade failed on-chain: `, trade.toString());
        return (Response as any).json({ message: "Failed to create transfer" }, { status: 500 });
      }
